    ...
    return (
      <div className={containerClass}>
        ...
        <div className={cardClass}>
          ...
          {/* Souhrn týdne nad seznamem záznamů */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            <div className="bg-indigo-50 rounded-xl p-4 shadow-sm">
              <div className="text-sm text-gray-600">Nákup (CZK)</div>
              <div className="text-2xl font-bold">{filteredTransactions.reduce((s,t)=> s + (parseFloat(t.purchasePricePln||0)*exchangeRate),0).toFixed(2)}</div>
            </div>
            <div className="bg-emerald-50 rounded-xl p-4 shadow-sm">
              <div className="text-sm text-gray-600">Prodej (CZK)</div>
              <div className="text-2xl font-bold">{filteredTransactions.reduce((s,t)=> s + (parseFloat(t.sellingPriceCzk||0)),0).toFixed(2)}</div>
            </div>
            <div className="bg-yellow-50 rounded-xl p-4 shadow-sm">
              <div className="text-sm text-gray-600">Čistý zisk (CZK)</div>
              <div className="text-2xl font-bold">{filteredTransactions.reduce((s,t)=> s + (parseFloat(t.netProfitCzk||0)),0).toFixed(2)}</div>
            </div>
  ...
  const SalesGraphView = ({ transactions, sellers }) => {
    ...
    useEffect(() => {
      ...
      const weeklyTransactions = transactions.filter(t => {
        ...
        if (t.saleDate) {
          transactionDate = new Date(t.saleDate);
